...
- I found another huge issue, and fixing this one does not seem so straightforward. I don't even know where to begin. The issue is that my 5 volt power supply works as intended when the PCB is not plugged into a VNA, but when it is plugged in, the 5 volt power supply outputs flat 0 volts. What is even more interesting is that the 11 volt power supply works regardless of the VNA being plugged in or not.
  - Aha....... I double checked the datasheet of the LNA and as it turns out, I only grounded the pins that are labeled as "NC" (No Connection). When I downloaded the footprint for the chip, the pins on the layout said that those should be grounded. When I asked Altium to do a polygon pour, it for some reason (probably to avoid islands) just did not connect the big center conductor of the chip, which happens to be the only place where the chip is connected to ground. This is probably why when I connect the VNA, the amplifier latches up because of no ground connection, and thus its associated power supply enters safety mode and turns off (the feedback diode turns on to reduce the output voltage to zero).
  - The only solution is likely to flatten some enamel wire (magnet wire) with a very small diameter (34 AWG ~= 6 mils) and place it to connect the grounded NC pins and the center conductor.
- Otherwise, below are the S-parameters of the Tx and Rx lines as of 5/5/2025:
  - <img src="https://github.com/user-attachments/assets/3240dffb-b501-4442-917f-5be5dbb5b77f" width="275"></img><img src="https://github.com/user-attachments/assets/961a9aa5-2582-41eb-9c2b-dd703a2a8ecd" width="250"></img>
  - It is clearly visible that the Rx line provides no gain, indicating the amplifier is not functioning.
